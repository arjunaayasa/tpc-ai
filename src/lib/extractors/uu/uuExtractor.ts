/**
 * UU Extractor - Parse UU (Undang-Undang) documents
 * UU has hierarchical structure: BAB → Bagian → Paragraf → Pasal → Ayat → Huruf
 * Plus optional PENJELASAN section
 * 
 * This extractor reuses PERPU parsing logic since they have identical structures.
 */
...
import { UuIdentity, UuSection, UuChunk, UuParseResult, UuChunkType, SourcePart } from './uuTypes';
 ...
 */
export function cleanUuText(rawText: string): string {
    let text = rawText;
...
/**
 * Extract UU identity (nomor, tahun, tentang)
 */
export function extractUuIdentity(text: string): UuIdentity {
    ...
    const nomorMatch = text.match(UU_NOMOR_REGEX);
    if (nomorMatch) {
        nomor = nomorMatch[1];
    ...
    return { nomor, tahun, tentang, tanggalDitetapkan, tanggalDiundangkan };
}
...
function generateAnchorCitation(
    ...
    const base = identity.nomor && identity.tahun
        ? `UU ${identity.nomor}/${identity.tahun}`
        : 'UU';
...
function splitBatangTubuhPenjelasan(text: string): DocumentParts {
    // Look for PENJELASAN header (UU-specific first, then generic)
    const penjelasanMatch = text.match(PENJELASAN_ATAS_UU_REGEX) || text.match(PENJELASAN_REGEX);
...
function findPasalBoundaries(text: string): PasalBoundary[] {
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        // Check if match starts with quote - this is definitively a header (quoted article text)
        const hasQuoteBefore = /["']\s*Pasal/i.test(match[0]);

        // More forgiving header detection - newline, ayat marker, or text continuation
        const looksLikeHeader = /^\s*(\n|\\n|\(1\)|Yang|Dalam|Setiap|$)/i.test(afterMatch);

        // If has quote before, always treat as header. Otherwise check reference + header heuristics
        if (hasQuoteBefore || !isReference || looksLikeHeader) {
...
/**
 * Parse a UU document into structured sections and chunks
 */
export function parseUu(rawText: string): UuParseResult {
    // 1. Clean text
    const cleanedText = cleanUuText(rawText);

    // 2. Extract identity
    const identity = extractUuIdentity(cleanedText);
    console.log(`[UU] Extracted identity: ${JSON.stringify(identity)}`);

    // 3. Split into BATANG_TUBUH and PENJELASAN
    const { batangTubuh, penjelasan } = splitBatangTubuhPenjelasan(cleanedText);
    console.log(`[UU] BATANG_TUBUH: ${batangTubuh.length} chars, PENJELASAN: ${penjelasan?.length || 0} chars`);

    // 4. Parse BATANG_TUBUH
    const batangTubuhChunks = parseBatangTubuh(batangTubuh, identity, 0);
    console.log(`[UU] Parsed ${batangTubuhChunks.length} BATANG_TUBUH chunks`);

    // 5. Parse PENJELASAN (if exists)
    let penjelasanChunks: UuChunk[] = [];
    if (penjelasan) {
        penjelasanChunks = parsePenjelasan(penjelasan, identity, batangTubuhChunks.length);
        console.log(`[UU] Parsed ${penjelasanChunks.length} PENJELASAN chunks`);
    }
    ...
    console.log(`[UU] Total chunks: ${allChunks.length}`);
