/**
 * PP Extractor - Parse PP (Peraturan Pemerintah) documents
 * Handles: BATANG_TUBUH (main body) and PENJELASAN (explanation) sections
 */
...
import { PpIdentity, PpSection, PpChunk, PpParseResult, PpChunkType, SourcePart } from './ppTypes';
 ...
 */
export function cleanPPText(rawText: string): string {
    let text = rawText;
...
/**
 * Extract PP identity (nomor, tahun, tentang)
 */
export function extractPPIdentity(text: string): PpIdentity {
    ...
    const nomorMatch = text.match(PP_NOMOR_REGEX);
    if (nomorMatch) {
        nomor = nomorMatch[1];
...
function generateAnchorCitation(
    ...
    options?: { pasal?: string; ayat?: string }
): string {
    const base = identity.nomor && identity.tahun
        ? `PP ${identity.nomor}/${identity.tahun}`
        : 'PP';
...
function splitBatangTubuhPenjelasan(text: string): DocumentParts {
    // Look for PENJELASAN header
    const penjelasanMatch = text.match(PENJELASAN_REGEX) || text.match(PENJELASAN_ATAS_REGEX);
...
/**
 * Parse a PP document into structured sections and chunks
 */
export function parsePP(rawText: string): PpParseResult {
    // 1. Clean text
    const cleanedText = cleanPPText(rawText);

    // 2. Extract identity
    const identity = extractPPIdentity(cleanedText);
    console.log(`[PP] Extracted identity: ${JSON.stringify(identity)}`);

    // 3. Split into BATANG_TUBUH and PENJELASAN
    const { batangTubuh, penjelasan } = splitBatangTubuhPenjelasan(cleanedText);
    console.log(`[PP] BATANG_TUBUH: ${batangTubuh.length} chars, PENJELASAN: ${penjelasan?.length || 0} chars`);

    // 4. Parse BATANG_TUBUH
    const batangTubuhChunks = parseBatangTubuh(batangTubuh, identity, 0);
    console.log(`[PP] Parsed ${batangTubuhChunks.length} BATANG_TUBUH chunks`);

    // 5. Parse PENJELASAN (if exists)
    let penjelasanChunks: PpChunk[] = [];
    if (penjelasan) {
        penjelasanChunks = parsePenjelasan(penjelasan, identity, batangTubuhChunks.length);
        console.log(`[PP] Parsed ${penjelasanChunks.length} PENJELASAN chunks`);
    }
    ...
    // 7. Build sections
    const sections: PpSection[] = [];
    // (Simplified - could add more detailed sections if needed)
    ...
    console.log(`[PP] Total chunks: ${allChunks.length}`);
